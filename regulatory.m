...
for k=6:kk
    ...
    if strcmp(algorytm, 'NPL') || strcmp(algorytm, 'GPC') || strcmp(algorytm, 'NO')
        ...
        if strcmp(algorytm, 'NPL') || strcmp(algorytm, 'NO')
            wesn_arx = [u(k-3) u(k-3) y(k-1) y(k-2)]';
            ymod(k)  = w20+w2*tanh(w10+w1*wesn_arx);
        elseif strcmp(algorytm, 'GPC')
            ymod(k)  = b(4)*u(k-3) + b(5)*u(k-3) + a(1)*y(k-1) + a(2)*y(k-2);
        end
...
title({'Wyj�cie regulatora',str, str2}); xlabel('k'); ylabel('warto�� sygna�u');
legend('wyj�cie y(k)','warto�� zadana');

subplot(2,1,2); 
stairs(u);hold on
title({'Sygna� sterowania'}); xlabel('k'); ylabel('warto�� sygna�u'); xlim([0, kk-N]);
